...
export default function GlobalSearch({ open, onClose }: Props) {
  ...
  return (
    <div style={{ position:'fixed', inset:0, zIndex:9000, background:'#000a', display:'flex', flexDirection:'column', padding:'16px 14px' }}>
      <div style={{ display:'flex', gap:8, alignItems:'center', marginTop:'env(safe-area-inset-top)' }}>
        <input
          ref={inputRef}
          style={{ flex:1, background:'var(--bg2)', border:'1px solid var(--border)', color:'var(--txt)', padding:'12px 16px', borderRadius:12, fontSize:14, fontFamily:"'DM Mono',monospace", outline:'none' }}
        ...
        />
        <button onClick={onClose} style={{ background:'var(--bg3)', border:'1px solid var(--border)', color:'var(--txt2)', padding:'10px 14px', borderRadius:10, cursor:'pointer', fontSize:13, flexShrink:0 }}>✕</button>
      </div>

      <div style={{ flex:1, overflowY:'auto', marginTop:10 }}>
        {!q.trim()
          ? <div style={{ textAlign:'center', padding:40, color:'var(--txt4)', fontSize:13 }}>Ketik nama untuk mencari...</div>
          : results.length === 0
            ? <div style={{ textAlign:'center', padding:40, color:'var(--txt4)', fontSize:13 }}>Tidak ditemukan</div>
            : results.map(r => (

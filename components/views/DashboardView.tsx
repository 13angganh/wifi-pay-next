...
export default function DashboardView() {
  ...
  function PctBadge({pct}:{pct:number|null}) {
    ...
    const up = pct>=0;
    return <span style={{fontSize:9,fontWeight:600,color:up?'#4CAF50':'#e05c5c',marginLeft:4}}>{up?'▲':'▼'}{Math.abs(pct)}% vs {MONTHS[prevDm]}</span>;
  }
    ...
    <div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:8}}>
        {([['KRS','#2196F3',krsTotal,krsPct2,krsLunas,krsAll.length,krsPct],
           ['SLK','#e05c3a',slkTotal,slkPct2,slkLunas,slkAll.length,slkPct]] as const).map(
          ([zone,color,total,pct2,lunas,allLen,pct])=>(

...
// FIXED: onBlur bug, lock banner position, lag/delay
'use client';
...
export default function MemberCard({ name, index }: Props) {
  ...
  // Track apakah input sudah dimodifikasi user — cegah onBlur palsu
  ...
  // Reset dirty flag saat card collapse
  useEffect(() => {
    if (!isExp) {
  ...
  // FIXED: hanya simpan jika user benar-benar mengetik (inputDirty)
  // dan tidak sedang collapsing
  async function saveEntryPay(rawVal: string) {
    if (!inputDirty.current) return;  // ← FIX UTAMA: cegah onBlur palsu
    ...
    if (rawVal === '' || rawVal === null) {
      ...
      await persist(newData,
        `🗑️ Hapus bayar ${activeZone} - ${name}`,
        `${MONTHS[cardMonth]} ${cardYear}: dihapus`
      );
      ...
      await persist(newData,
        `💰 Bayar ${activeZone} - ${name}`,
        `${MONTHS[cardMonth]} ${cardYear}: ${amt === 0 ? 'Akumulasi' : rp(amt)}`
      );
  ...
  async function saveDate(dateVal: string) {
    ...
    await persist(
      ...
      `📅 Update tanggal ${activeZone} - ${name}`,
      `${MONTHS[cardMonth]} ${cardYear}: ${dateVal}`
    );
  }
  ...
  if (freeCur)
    tagEl = <span className="mc-tag" style={{ background:'#0a3a25',color:'#4CAF50',border:'1px solid #4CAF5033',fontSize:9 }}>🆓</span>;
  else if (val !== null && val > 0)
    tagEl = <span className="mc-tag tpaid">✓</span>;
  else if (val === 0)
    tagEl = <span className="mc-tag" style={{ background:'#0a2a1a',color:'#3a9e7a',border:'1px solid #3a9e7a44',fontSize:9 }}>✓ 0</span>;
  else
    tagEl = <span className="mc-tag tunpaid">✕</span>;
  ...
  return (
    <>
      <div ref={cardRef} id={`card-${name.replace(/\s/g,'_')}`} className={`mcard ${isExp ? 'expanded' : ''}`}>
        {/* Top row */}
        <div className="mc-top" onClick={handleToggle}>
              ...
              <>
                ...
                <div className="mc-row">
                  ...
                  <input
                    ...
                    onChange={() => { inputDirty.current = true; }}  // ← tandai user sudah ketik

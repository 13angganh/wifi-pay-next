...
import { doJSONBackup } from '@/lib/export';
    ...
    <>
      <div id="header">
        ...
        <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:8 }}>
          <button id="hamburger" className="hbtn" style={{ padding:'5px 8px', flexShrink:0 }} onClick={onToggleSidebar}>☰</button>
          <div style={{ display:'flex', alignItems:'center', gap:8 }}>
            <div style={{ width:30, height:30, borderRadius:8, background:`linear-gradient(135deg,${zc},${zc}bb)`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:14, flexShrink:0 }}>📶</div>
            <div>
              <div style={{ fontFamily:"'Syne',sans-serif", fontWeight:800, fontSize:14, letterSpacing:'-.02em' }}>WiFi Pay</div>
        ...
        <div style={{ display:'flex', gap:6, alignItems:'center', flexWrap:'wrap' }}>
          <span style={{ display:'flex', alignItems:'center', gap:4 }}>
            ...
            <span style={{ fontSize:10, color:'var(--txt4)' }}>{syncLabel[syncStatus]??syncStatus}</span>
          </span>
          ...
          <button className="hbtn"
            ...
            {globalLocked?'🔒':'🔓'}<span style={{ fontSize:9 }}>{globalLocked?'KUNCI':'BUKA'}</span>
          </button>
          <button className="hbtn" onClick={() => setSearchOpen(true)}>🔍</button>
          <button className="hbtn" onClick={toggleTheme}>{darkMode?'🌙':'☀️'}</button>
          <button className="hbtn" onClick={() => setAccountOpen(true)}>👤</button>
        </div>
      </div>
